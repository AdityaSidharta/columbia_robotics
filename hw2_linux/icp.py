...
def obj_depth2pts(obj_id, depth, mask, camera, view_matrix):
    ...
    world_pts = transform_point3s(np.linalg.inv(campose), camera_pts)
    return world_pts
...
def estimate_pose(depth, mask, camera, view_matrix):
    ...
    for obj_id in [1, 2, 3, 4, 5]:
        ...
        tfm_matrix = align_pts(mesh_pts, est_pts)
...
def main():
    ...
    for scene_id in range(5):
        ...
        export_gt_ply(scene_id, depth, pred_mask, my_camera, view_matrix)
